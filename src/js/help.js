// help.js - Keyboard shortcut help panel
import { getAppVersion, getMode } from './core/keymode.js';

let helpPanel = null;

const sections = [
  {
    title: '表示・レイアウト',
    items: [
      ['Ctrl+Shift+J', 'エディタのみ表示'],
      ['Ctrl+Shift+K', '分割表示'],
      ['Ctrl+Shift+L', 'プレビューのみ表示'],
      ['Ctrl+| / Ctrl+Shift+D', '縦分割'],
      ['Ctrl+\\ / Ctrl+Shift+H', '横分割'],
      ['Ctrl+Shift+W', 'タブ/ペインを閉じる（複数ペインならペイン優先）'],
      ['Ctrl+矢印', 'ペイン移動'],
      ['Ctrl+- / Ctrl++', 'エディタの文字 縮小 / 拡大'],
      ['Ctrl+Shift+- / Ctrl+Shift++', 'アプリ全体の文字 縮小 / 拡大'],
      ['Ctrl+Shift+B', 'メニューバーの表示/非表示'],
      ['Alt（単押し）', 'メニューバーを開く（非表示でも一時的に開く）'],
      ['Alt+F / E / I / V / A / H', 'ファイル / 編集 / 挿入 / 表示 / AI / ヘルプ を開く'],
      ['メニュー内 ↑↓ / ←→ / Enter / Esc', '項目移動 / 隣のメニュー / 実行 / 閉じる'],
      ['Ctrl+Shift+E', 'サイドバーへフォーカス（ファイラ⇄アウトラインを巡回）'],
      ['↑ / ↓', 'ファイラ/アウトライン内で項目移動'],
      ['→ / ←', 'ファイラでフォルダを開く / 閉じる・親へ'],
      ['Enter', 'ファイラ/アウトラインの項目を開く'],
      ['Esc', 'サイドパネル/AIパネルからエディタへ戻る'],
    ],
  },
  {
    title: 'ファイル・タブ',
    items: [
      ['Ctrl+Shift+T / Ctrl+Shift+N', '新規タブ'],
      ['Ctrl+Tab / Ctrl+PageDown', '次のタブ'],
      ['Ctrl+Shift+Tab / Ctrl+PageUp', '前のタブ（Linuxでは Ctrl+PageUp 推奨）'],
      ['Ctrl+S', '保存（Emacsモードは C-x C-s）'],
      ['Ctrl+Shift+S', '名前を付けて保存'],
      ['Ctrl+Shift+O', 'フォルダを開く'],
      ['Ctrl+Shift+U', 'ファイラでこのファイルの場所を表示（ツリー外ならそのフォルダを開く）'],
      ['Ctrl+Shift+R', 'ファイルをディスクから再読込（外部変更を検知すると自動）'],
    ],
  },
  {
    title: '編集（Markdown）',
    items: [
      ['Ctrl+B', '太字トグル（Normal/Vim）'],
      ['Ctrl+Shift+8', '箇条書きトグル'],
      ['Ctrl+Shift+7', '番号付きリストトグル'],
      ['Ctrl+F', '検索・置換（Normal/Vim）'],
      ['Ctrl+Shift+V', 'そのまま貼り付け（表への自動変換をしない）'],
    ],
  },
  {
    title: '表',
    items: [
      ['Ctrl+Shift+G', '表を挿入（行数・列数をグリッドで指定）'],
      ['Tab / Shift+Tab', '次 / 前のセルへ移動（行末では新しい行を追加）'],
      ['Enter', '同じ列の下のセルへ移動'],
      ['Ctrl+Shift+F', 'カーソル位置の表を整形'],
      ['右クリック → Convert to Table', '選択範囲を表に変換（メニュー「挿入」からも実行可）'],
      ['貼り付け', 'タブ区切り（表計算からのコピー）のみ自動で表に変換'],
    ],
  },
  {
    title: 'AI',
    items: [
      ['Ctrl+Shift+I', 'AIチャットを開く＋入力欄へフォーカス（パネル内で再押下＝閉じる）'],
      ['Ctrl+Shift+C', 'AIコンポーザー（選択範囲を編集）'],
      ['Tab', 'インライン補完を受け入れる'],
      ['Esc', 'インライン補完を破棄'],
      ['右クリック', 'AIアクションメニュー（選択範囲が必要）'],
    ],
  },
  {
    title: 'キーモード',
    items: [
      ['Ctrl+Shift+M', 'モード切替（Normal → Vim → Emacs）'],
      ['設定パネル', 'モードを直接選択（Ctrl+,）'],
    ],
  },
  {
    title: 'Vimモード',
    items: [
      ['jj / jk', 'インサートモード解除（ESC代替）'],
      ['Ctrl+[', 'インサート/ビジュアル解除（ESC代替）'],
    ],
  },
  {
    title: 'Emacsモード（カーソル/編集）',
    items: [
      ['Ctrl+A / Ctrl+E', '行頭 / 行末'],
      ['Ctrl+B / Ctrl+F', '1文字前 / 後'],
      ['Ctrl+P / Ctrl+N', '前行 / 次行'],
      ['Ctrl+D / Ctrl+H', '次文字を削除 / 前文字を削除'],
      ['Ctrl+K / Ctrl+Y', '行末までキル / ヤンク（貼り付け）'],
      ['Ctrl+W / Alt+W', '領域をキル / コピー'],
      ['Ctrl+S / Ctrl+R', 'インクリメンタル検索 / 逆検索'],
      ['Alt+B / Alt+F', '単語前 / 後'],
      ['Alt+V / Ctrl+V', 'ページ前 / 後'],
    ],
  },
  {
    title: 'その他',
    items: [
      ['Ctrl+,', '設定'],
      ['Ctrl+?', 'このヘルプ'],
    ],
  },
  {
    title: 'ブラウザモード用',
    items: [
      ['Alt+N / Alt+T', '新規タブ'],
      ['Alt+W', 'タブ/ペインを閉じる'],
      ['Alt+O', 'フォルダを開く'],
    ],
  },
];

export function openHelp() {
  if (helpPanel) return;

  helpPanel = document.createElement('div');
  helpPanel.className = 'help-overlay';

  const sectionsHtml = sections
    .map(({ title, items }) => {
      const rows = items
        .map(([key, desc]) => `<tr><td><kbd>${key}</kbd></td><td>${desc}</td></tr>`)
        .join('');
      return `<section class="help-section"><h3>${title}</h3><table><tbody>${rows}</tbody></table></section>`;
    })
    .join('');

  const ver = getAppVersion();
  const mode = getMode();
  const aboutHtml = `
    <section class="help-section help-about">
      <h3>About</h3>
      <table><tbody>
        <tr><td>Version</td><td>${ver ? `Fude v${ver}` : 'Fude (unknown version)'}</td></tr>
        <tr><td>Current key mode</td><td>${mode}</td></tr>
      </tbody></table>
    </section>`;

  helpPanel.innerHTML = `
    <div class="help-panel">
      <div class="help-header">
        <h2>Keyboard Shortcuts</h2>
        <button class="help-close icon-btn">\u00d7</button>
      </div>
      <div class="help-body">
        ${aboutHtml}
        ${sectionsHtml}
      </div>
    </div>
  `;

  document.body.appendChild(helpPanel);

  helpPanel.querySelector('.help-close').addEventListener('click', closeHelp);
  helpPanel.addEventListener('click', (e) => {
    if (e.target === helpPanel) closeHelp();
  });

  const handleEsc = (e) => {
    if (e.key === 'Escape') {
      closeHelp();
      document.removeEventListener('keydown', handleEsc);
    }
  };
  document.addEventListener('keydown', handleEsc);
}

export function closeHelp() {
  if (helpPanel) {
    helpPanel.remove();
    helpPanel = null;
  }
}
